// ==========================================
// TOAST NOTIFICATION KHUSUS LOGIN
// ==========================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    formForgot.addEventListener('submit', (e) => {
        ...
        if (!usersDB[fEmail]) { showToast("❌ Email tersebut belum pernah didaftarkan!", "error"); return; }

        usersDB[fEmail].status = 'reset_pending';
        usersDB[fEmail].tempPassword = fPass;
        localStorage.setItem('revpro_users_db', JSON.stringify(usersDB));

        modalForgot.classList.add('hidden');
        showToast("⏳ Pengajuan reset sandi berhasil! Silakan tunggu ACC dari Admin.", "success");
        
        // UPDATE EMAIL ADMIN DISINI
        setTimeout(() => {
    ...
    btnCheckStatus.addEventListener('click', (e) => {
        e.preventDefault();
        statusResult.classList.add('hidden'); // Sembunyikan hasil lama
    ...
    formStatus.addEventListener('submit', (e) => {
        ...
        statusResult.classList.remove('hidden', 'bg-yellow-100', 'text-yellow-800', 'border-yellow-300', 'bg-red-100', 'text-red-800', 'border-red-300', 'bg-green-100', 'text-green-800', 'border-green-300', 'bg-blue-100', 'text-blue-800', 'border-blue-300', 'bg-gray-100', 'text-gray-800', 'border-gray-300');
        ...
        if (!usersDB[cEmail]) {
            ...
            const status = usersDB[cEmail].status;
            if (status === 'pending') {
    ...
    formUser.addEventListener('submit', function(e) {
        ...
        if (!usersDB[email]) {
            // USER BARU
            usersDB[email] = { password: password, status: 'pending' };
            localStorage.setItem('revpro_users_db', JSON.stringify(usersDB));
            showToast(`📩 Akun ${email} berhasil didaftarkan! Status: PENDING.`);
    ...
    formAdmin.addEventListener('submit', function(e) {
        ...
        // UPDATE KREDENSIAL ADMIN DISINI
